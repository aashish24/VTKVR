...
\item \emph{Camera Inside the Bounding Box}
	Camera views within the volume are handled by clipping the proxy geometry
    with the camera's near plane (using \texttt{vtkClipConvexPolyData}), thus
    ensuring all the bounding box fragments fall within range.
