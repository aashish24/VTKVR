...
~\Autoref{fig:frontandback}. Using the interpolated vertex position and
...
skeleton fragment shader defines tags as shown in~\Autoref{lst:skeletonshader}.
...
\centering
  ...
  \end{subfigure}
  \caption{Holland cactus dataset \emph{needCitation} exhibiting wood-grain
  artifacts (a).  Same rendered image but with stochastic jittering enabled
  to mitigate the artifacts (b).}
  \label{fig:jittering}
\end{figure}
